...
<!-- UP TO HERE - Error on following function call: 'Entity nbsp' not defined -->
<!-- Check currency of ABS API - Try: https://api.data.abs.gov.au/dataflow -->
...
```{r eval=FALSE}
sel_releases <- abs_cat_releases(sel_title$title[1]) %>%
  filter(grepl("National.+Income.+Expenditure", title, ignore.case=TRUE))
```
...
```{r eval=FALSE}
sel_tables <- abs_cat_tables(sel_title$title, releases="Latest");
```
...
```{r eval=FALSE}
sel_data <- abs_cat_stats(sel_title$title, releases="Latest", tables="Table\\s*(1|2)\\.");
```
...
```{r echo=FALSE}
knitr::kable(ana_releases %>% slice_head(n=10))
```
...
```{r echo=FALSE}
knitr::kable(cpi_releases %>% slice_head(n=10))
```
...
Expenditure, Australia* (ABS Catalogue no. 5265.0).

First download selected tables (1, 6 and 11-16):
```{r message=FALSE, results=FALSE}
capex_q <-
  abs_cat_stats("Private New Capital Expenditure and Expected Expenditure, Australia",
                tables=c(
                  ## Table 1 - Actual Expenditure, by Type of Asset and Industry, Original - Current Prices
                  "01_current_prices_original",
                  ## Table 6 - Actual Expenditure, By Type of Industry, Original - Chain Volume Measures
                  "06_volume_measures_original",
                  ## Tables 11-16 Actual and Expected Capital Expenditure by Industry, State, Current Prices"
                  sprintf("1(%s)_current_prices.+_capex", paste(1:6, collapse="|"))));
```

Then add a variable identifying Australian state/territory.
...
capex_q <- capex_q %>%
  mutate(state = sub(sprintf(".*(%s).*",
           ...
           factor(levels=c("New South Wales", "Victoria", "Queensland",
                           "South Australia", "Western Australia",
                           "Tasmania", "Total (State)")))
```
...
capex_q_min <- capex_q %>%
  ...
  filter(!grepl("non-mining", data_item_description, ignore.case=TRUE)) %>%
