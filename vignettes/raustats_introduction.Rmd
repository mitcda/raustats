...
The functions in this package help facilitate the download and transformation of
ABS and RBA statistical datasets into long-format (tidy) tables.

The ABS is also developing a data access API:
[ABS.Stat](http://stat.data.abs.gov.au/), which is currently in Beta release and
provides access to a subset of ABS statistics. This package include functions to
search and download data via the [ABS.Stat](http://stat.data.abs.gov.au/)
API. Please note that as the API is still in development, changes in the API
back-end may break the functions provided in this package.
...
The `abs_cat_stats` function is the workhorse for downloading ABS statistics by
ABS Catalogue Number. For example, the latest Consumer Price Index (CPI) data
series (ABS Catalogue no. 6401.0) can be downloaded simply with the following
call:
...
Alternatively, you can provide a regular expression to `tables` that matches the
table name as specified on the ABS data access page, as follows:
...
API.^[Note that, as at the time of writing, the ABS.Stat API had a limit on
query string length and number of records returned.]  In order to download the
latest CPI data series for All Group changes in prices across Australia and each
of the eight capital cities, simply call:
...
A more detailed explanation of the `rba_stats` and other RBA data access
...
provide any functionality to access this facility. Instead, the `abs_tablecache`
...
```{r eval=FALSE}
## December 2017 and 2016 quarter national accounts - Table 1
ana_Q4 <- abs_cat_stats(cat_no="5206.0", tables="Table 1", releases=c("Dec 2017","Dec 2016"));
```

The `abs_cat_stats` function is designed to download both ABS
time series spreadsheets (`types="tss"`) and cross-section spreadsheets
(`types="css"`)---'Data Cubes' in ABS terminology.  ABS time series
spreadsheets generally have a standard format, with a single column for each
series, several headers rows containing series metadata and a single row with
the unique series identifier.  ABS cross-section spreadsheet formats vary,
...
(cross-section spreadsheets, `types="css"`) is planned to be added in the near
future.
...
The `abs_cat_unzip` function extracts Excel files from compressed ABS
zip archives, it uses the `utils::unzip` function (using some standard
file locations). There are two arguments: `files` and `exdir` which
have a similar meaning to the `utils::unzip` equivalent arguments. By
default `exdir = tempdir()`.
...
simple example below shows use of the function to read Table 1 from the national
...
specified regular expression. The following examples demonstrates use of the
...
```{r}
## Find all dimensions in the CPI dataset matching the expressions 'All groups CPI' and 'Sydney'
...
with packages like `ggplot2`, `tidyr` and `dplyr`. The following example
illustrates how data downloaded using the `raustats` can be easily transformed
and plotted. This example uses data from the ABS' *Private New Capital
Expenditure and Expected Expenditure* (ABS Catalogue no. 5265.0).
...
The following plot shows actual mining sector capital expenditure by Australian
state and territory.

```{r fig.height=4, fig.width=7, fig.path="figures/", error=TRUE, message=TRUE, verbose=TRUE}
library(ggplot2)
### Filter actual mining capital expenditure
capex_q_min <- capex_q %>%
...
ggplot(data=capex_q_min) +
  ...
  labs(title="Australian mining capital expenditure, by state",
...
[ABS.Stat](http://stat.data.abs.gov.au/) API is, as at the time of writing,
still in Beta and subject to revision, some of these issues will be addressed as
[ABS.Stat](http://stat.data.abs.gov.au/) transitions to a Release version in
future versions of the `raustats` package.


## Searching in other languages

The [ABS.Stat](http://stat.data.abs.gov.au/) API (Beta version) appears to
include scope to cater to languages other than English. At the time of writing,
French appears to be the only other language included in the data sets, however
the text for many records denoted as French appear to be in English. The ABS API
...
clear whether the session limit applies. If time limits are an issue, the ABS
